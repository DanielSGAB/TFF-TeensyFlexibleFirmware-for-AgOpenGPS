...
// Part of TFF v0.3.7. Was "inherited unchanged from UM982 Fallback
// Firmware v0.6/v0.7" through v0.2 — no longer accurate as of v0.3:
// CAN_setup() gained a Keya-specific V_Bus FIFO filter (index 5,
// independent of Brand — see the filter's own comment for why that
// index) and its own call site's gating condition was widened to also
// run when MotorDriveType == MOTOR_DRIVE_KEYA even with Brand ==
// BRAND_NONE (the typical case for a Keya installation, which has no
// CAN-ready tractor brand at all). Every brand-specific line in this
// file is untouched — see that project's own files for the full v0.1-
// v0.7 history of everything else here. VBus_Send() call site (in the
// main .ino's loop()) is rate-limited to ~50ms — no changes were
// needed in this file itself for that fix.
//
//  Brand is selected at runtime via SETBRAND:N over Teensy Tool
//  (UDP port 5556, saved to EEPROM — see receiveMonitorCommands() in
//  zHandlers.ino), NOT via a serial Service Tool menu — this project
//  does not include CAN_Service_Tool.ino from the upstream repo.
//  Reboot the Teensy after changing Brand so CAN filters re-init.
//  0 = Claas (1E/30 Navagation Controller, 13/19 Steering Controller)
//      — for Claas-specific notes, see the upstream project's
//      "Service Tool" documentation page (external, not included here)
...
//V_Bus is CAN-3 and is the Steering BUS
  ...
  // v0.3.11 — replaces every previously HARDCODED filter index
  // (0, 1, 2, ...) below with this single, running counter instead.
  // ROOT CAUSE this fixes: a real, field-found bug (a tester's own
  // added Valtra armrest-engage filter used index 5, silently
  // colliding with Keya's OWN, separately hardcoded index-5
  // reservation a few lines below — two SEPARATE pieces of code each
  // independently guessing a "safe" number, with no way for either to
  // know if the other had changed). FlexCAN_T4's FIFO filter table
  // has a hard limit of 8 slots total (indices 0-7, confirmed via the
  // library's own PJRC forum documentation) — not something we can
  // configure our way out of, only coordinate around correctly.
  // With this counter, NO index anywhere in this function is ever
  // hardcoded again — every setFIFOFilter() call below uses
  // "nextFilterIndex++" instead of a literal number, so adding a
  // filter to any Brand block (exactly what the tester did) can never
  // silently collide with Keya's reservation, or with any OTHER
  // Brand's filters, ever again — there is only ever one thing
  // deciding "what's the next free slot", not two independently
  // guessing.
  uint8_t nextFilterIndex = 0;

if (Brand == 0){
  V_Bus.setFIFOFilter(nextFilterIndex++, 0x0CAC1E13, EXT);  //Claas Curve Data & Valve State Message
  V_Bus.setFIFOFilter(nextFilterIndex++, 0x18EF1CD2, EXT);  //Claas Engage Message
  V_Bus.setFIFOFilter(nextFilterIndex++, 0x1CFFE6D2, EXT);  //Claas Work Message (CEBIS Screen MR Models)
  }
if (Brand == 1){
    V_Bus.setFIFOFilter(nextFilterIndex++, 0x0CAC1C13, EXT);  //Valtra Curve Data & Valve State Message
    V_Bus.setFIFOFilter(nextFilterIndex++, 0x18EF1C32, EXT);  //Valtra Engage Message
    V_Bus.setFIFOFilter(nextFilterIndex++, 0x18EF1CFC, EXT);  //Mccormick Engage Message
    V_Bus.setFIFOFilter(nextFilterIndex++, 0x18EF1C00, EXT);  //MF Engage Message
    V_Bus.setFIFOFilter(nextFilterIndex++, 0x18FF8306, EXT);  //Mccormick Joystick
  }  
if (Brand == 2){
    V_Bus.setFIFOFilter(nextFilterIndex++, 0x0CACAA08, EXT);  //CaseIH Curve Data & Valve State Message
    V_Bus.setFIFOFilter(nextFilterIndex++, 0x18FFBB03, EXT);  //CaseIH Engage Message
  }   
if (Brand == 3){
  V_Bus.setFIFOFilter(nextFilterIndex++, 0x0CEF2CF0, EXT);  //Fendt Curve Data & Valve State Message
  }   
if (Brand == 4){
  V_Bus.setFIFOFilter(nextFilterIndex++, 0x0CACAB13, EXT);  //JCB Curve Data & Valve State Message
  V_Bus.setFIFOFilter(nextFilterIndex++, 0x18EFAB27, EXT);  //JCB engage message
  }
if (Brand == 5){
  V_Bus.setFIFOFilter(nextFilterIndex++, 0x0CEF2CF0, EXT);  //FendtONE Curve Data & Valve State Message
  }   
if (Brand == 6){
  V_Bus.setFIFOFilter(nextFilterIndex++, 0x0CACF013, EXT);  //Lindner Curve Data & Valve State Message
  }
if (Brand == 7){
  V_Bus.setFIFOFilter(nextFilterIndex++, 0x0CAC1C13, EXT);  //AgOpenGPS Curve Data & Valve State Message
  V_Bus.setFIFOFilter(nextFilterIndex++, 0x19EF1C13, EXT);  //AgOpenGPS error message
  }   

// Keya CAN steering motor heartbeat — independent of Brand (a Keya
// installation typically has Brand == BRAND_NONE, where V_Bus would
// otherwise have zero active filters at all, just the REJECT_ALL
// above). Index now taken from nextFilterIndex — whatever the ACTIVE
// Brand block above actually used, not a hardcoded guess about what
// it MIGHT use (see this function's own opening comment for the real
// bug this replaces). NOT incremented further (no "++") — Keya's
// filter is always the LAST one this function sets, nothing after it
// needs the counter's value again this call.
//
// Defensive check: FlexCAN_T4's FIFO filter table has a hard limit of
// 8 slots (indices 0-7) — if some future Brand block ever grows to
// use all 8 on its own, there would genuinely be no room left for
// Keya at all. Rather than silently overwriting slot 7 (colliding
// with whatever that Brand's own last filter was), this logs a clear
// warning and skips setting Keya's filter entirely — a missing Keya
// filter is loud and diagnosable (motor simply never responds to
// heartbeats), a silent overwrite of a Brand's own filter would not
// be.
// 0x07000001 = heartbeat ID for Keya motor node address 1 (the
// factory-default node address — see keyaSetup() in zKeya.ino for the
// address-to-ID formula if a different node address is ever needed).
if (MotorDriveType == MOTOR_DRIVE_KEYA)
{
  if (nextFilterIndex <= 7)
  {
    V_Bus.setFIFOFilter(nextFilterIndex, 0x07000001, EXT);  //Keya steering motor heartbeat
  }
  else
  {
    Serial.println(F("WARNING: no free CAN filter slot left for Keya (Brand's own "
                      "filters already use all 8) — Keya heartbeat will not be "
                      "received, motor will not respond."));
  }
}
  V_Bus.setFIFOFilter(5, 0x07000001, EXT);  //Keya steering motor heartbeat
...
// Sends the CAN "release Go button" message — the counterpart to
// pressGo() above. See SetRelaysFendt()'s own block comment
// (c00_Autosteer.ino) for the full orchestration this and its
// siblings (pressGo/pressEnd/liftEnd/pressCSM1/pressCSM2) serve —
// this function specifically fires once goDown transitions back to
// false, releasing the headland "Go" button the tractor's own
// dashboard was virtually holding down.
void liftGo()
{
     CAN_message_t buttonData;
     buttonData.id = 0X613;
